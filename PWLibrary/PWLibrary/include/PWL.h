...
#define PWL_ShaderType int
#define PWL_SHADER_TYPE_ERROR 0
#define PWL_SHADER_TYPE_VERTEX 1
#define PWL_SHADER_TYPE_FRAGMENT 2
#define PWL_SHADER_TYPE_COMPUTE 3
...
/**
 * @brief 
 * The function callback for scene, it will loop in the scene.
 * change the char value in next_scene, when function return this char, it will change next scene.
 * @param next_scene you must change it, or the main scene.
 * @param delta the time between the two loops.
 * @return char* 
 */
char* ExampleEmptyLoop(char* next_scene, float delta, PWL_SceneLoopPackage* self)
{
        /*
         * // The delta will help you move something smoothly
         * element.x += delta * speed;
         * 
         */
        next_scene = ""; // if empty, mean no change.
        return next_scene;
}

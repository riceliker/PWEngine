...
namespace PWEngine::Stream::Config
{
    ...
    size_t
    ConfigFile::KeyHash::operator()(std::array<char, 64> buf) const noexcept
    {
    ...
    bool ConfigFile::KeyEqual::operator()(
        const std::array<char, 64>& a,
        const std::array<char, 64>& b) const noexcept
    {
        return std::strcmp(a.data(), b.data()) == 0;
    }
    ConfigFile::ConfigFile(std::string config_file_path, Log::LogSystem* log)
        : log(log)
    {
        /* path check */
        ...
        {
            log->send(Log::LogType::Warn,
                      Log::LogFrom::FileIO,
                      "Can not find config.txt file.");
        ...
        if (std::filesystem::is_directory(config_file_path))
        {
            log->send(Log::LogType::Warn,
                      Log::LogFrom::FileIO,
                      "The config.txt is dictionary. Not a file.");
            this->read_failed = true;
        }
        /* open file */
        ...
        while (std::getline(file, get_line))
        {
            ...
            if (key.length() > 63)
            {
                log->send(Log::LogType::Warn,
                          Log::LogFrom::FileIO,
                          "The key text length can not longer than 64 in "
                          "config.txt file.");
                return;
            }
            if (value.length() > 255)
            {
                log->send(Log::LogType::Warn,
                          Log::LogFrom::FileIO,
                          "The value text length can not longer than 256 in "
                          "config.txt file.");
                return;
            }
            /* store it */
    ...
    std::optional<std::string> ConfigFile::get(std::string key)
    {
        ...
        {
            std::string msg =
                std::format("Can no find key (%s) in config.txt file.", key);
    ...
    void ConfigFile::set(std::string key, std::string value)
    {
        ...
        if (value.length() > 255)
        {
            log->send(
                Log::LogType::Warn,
                Log::LogFrom::FileIO,
                "The value text length can not longer than 256 in config.txt "
                "file.");
            return;
        }
        std::array<char, 64> key_array{};
        ...
        {
            std::string msg =
                std::format("Can no find key ({}) in config.txt file.", key);
            log->send(Log::LogType::Warn, Log::LogFrom::FileIO, msg);
            return;
        }
        else
    ...
    void ConfigFile::write()
    {
        ...
        temp_file_path.replace_filename(std::string("~") +
                                        this->file_path.filename().string());
